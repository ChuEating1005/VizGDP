  ...
  export default {
    ...
    methods: {
      async loadDataAndRenderChart() {
        try {
          const [gnhResponse, lifeExpectancyResponse] = await Promise.all([
            axios.get('data/GNH_2023.csv'),
            axios.get('data/life-expectancy.csv')
          ]);
          const gnhData = this.parseCSV(gnhResponse.data);
          const lifeExpectancyData = this.parseLifeExpectancyCSV(lifeExpectancyResponse.data);
    
          // Filter GNH data for the year 2023 and match with life expectancy data
          this.allData = gnhData.filter(d => d.year === '2023').map(d => {
            if (!d.code) {
              return null; // Skip if country code does not exist
            }
            const lifeExpectancyEntry = lifeExpectancyData.find(le => le.code === d.code);
            if (!lifeExpectancyEntry) {
              return null; // Skip if no matching country code
            }
            return {
              ...d,
              lifeExpectancy: lifeExpectancyEntry.lifeExpectancy
            };
          }).filter(Boolean); // Remove null entries
    
          this.availableRegions = [
            ...new Set(this.allData.map(d => d.region).filter(Boolean)),
          ];
      ...
      parseLifeExpectancyCSV(csvData) {
        const rows = csvData.split('\n');
        const headers = rows[0].split(',');
        const yearIndex = headers.indexOf('2021');
        const data = rows.slice(1).map(row => {
          const values = row.split(',').map(v => (v === '' ? null : v.trim()));
          return {
            code: values[3],
            lifeExpectancy: values[yearIndex] ? parseFloat(values[yearIndex]) : null
          };
        });
        return data;
      },
      initializeChart() {
        ...
        const seriesData = this.allData.map(d => ({
          value: [d.logGDP, d.lifeExpectancy],
          ...
          tooltip: {
            ...
            formatter: function (params) {
              ...
              return `
                ...
                <div>Life expectancy: ${params.value[1].toFixed(2)} years old</div>
